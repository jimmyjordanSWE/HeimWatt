...
```c
// RULE: Only use direct mem_alloc if MEM_NEW/MEM_ALLOC_ARRAY macros don't fit.
// When using mem_alloc, always apply sizeof to the VARIABLE, never the TYPE.
// Why: If you change the variable's type later (e.g., float* to double*),
//      the allocation size automatically updates. Hardcoding type leads to insufficient allocation bugs.

// GOOD: Robust against refactoring (and mem_alloc zeroes memory)
weather_data* data = mem_alloc(sizeof(*data));

// BAD: Brittle - requires manual update if type changes
weather_data* data = mem_alloc(sizeof(weather_data));
```
...
> **Key Ownership Rules**
> 1.  **Tracked Allocations**: Always use `mem_alloc(sizeof(*ptr))`. NEVER use raw `malloc`.
> 2.  **Explicit Transfer**: Ownership transfer must be explicit.
> 3.  **Const by Default**: Borrowed pointers must be `const`.
> 4.  **Safe Destructors**: Destructors take `T**` and set the pointer to `NULL`.
> 5.  **Pool/Arena Usage**: Use `HwPool` for high-frequency objects and `HwArena` for request lifecycles.

#### Standard Allocation Strategy

We use a **Tracked Approach** for safety:
...
weather_data* weather_data_create(void) {
    // RULE: Use mem_alloc(sizeof(*ptr)) for type-safe allocation.
    // Why: Guarantees zero-initialization and is refactor-proof.
    weather_data* data = mem_alloc(sizeof(*data));
    if (!data) return NULL;
...
```c
// Use mem_alloc for arrays of fixed size
weather_data* slots = mem_alloc(count * sizeof(*slots));
// if (slots) ... zeroed by default
...
energy_plan* energy_plan_create(size_t count) {
    ...
    plan = mem_alloc(total_size);
    if (!plan) return NULL;
    // mem_alloc guarantees zeroed memory
    plan->count = count;
...
### Dynamic Buffers (HwBuffer)

For variable-length data (strings, IPC messages, HTTP bodies), **always** use `HwBuffer` from `memory.h` instead of raw `realloc` or fixed-size arrays.

**Why?**
1.  **Safety**: Automatically handles allocation, growth, and null-termination.
2.  **Security**: Ensures all new memory is `memset` to 0 (prevents info leaks and optimistic allocation).
3.  **Performance**: Uses geometric growth (2x) to minimize syscalls.

```c
#include "memory.h"

int build_message(const char* name) {
    HwBuffer buf;
    hw_buffer_init(&buf); // Stack allocation, init to safe state

    // 1. Append data safe and easy
    if (hw_buffer_append(&buf, "Hello, ", 7) < 0) goto cleanup;
    if (hw_buffer_append(&buf, name, strlen(name)) < 0) goto cleanup;

    // 2. Use buffer.data as C-string (guaranteed null-terminated)
    printf("Msg: %s\n", buf.data);

    // 3. Clear for reuse (keeps memory)
    hw_buffer_clear(&buf);

cleanup:
    hw_buffer_free(&buf); // Safe cleanup
    return 0;
}
```
...
| `alloca()` | Stack overflow | `mem_alloc()` (heap) |
| `system()` | Command injection | `exec*()` family |
| `malloc()` / `free()` | Unsafe, untracked | `mem_alloc()` / `mem_free()` (See memory.h) |
| `calloc()` / `realloc()` | Unsafe, untracked | `mem_alloc()` / `mem_realloc()` |
| `strdup()` | Untracked allocation | `mem_strdup()` |
...
**Why Use This?**
*   **Performance**: Avoids thousands of `mem_alloc`/`mem_free` calls per second.
*   **Throttling**: Naturally limits resource usage (e.g., max 50 DB connections).
*   **Fragmentation**: Prevents heap fragmentation for fixed-size objects.

```c
#include "memory.h"

// Define pool in your context
typedef struct {
    HwPool* conn_pool;
} server_ctx;

// Init
ctx->conn_pool = hw_pool_create(sizeof(connection_t), 100);

// Usage
connection_t* conn = hw_pool_alloc(ctx->conn_pool);
// use conn...
hw_pool_free(ctx->conn_pool, conn);

// Cleanup
hw_pool_destroy(&ctx->conn_pool);
```

### Design Pattern 6: Request Arena (Scope-Based)
**Why Use This?**
*   **Zero-Overhead Cleanup**: Free everything in O(1) at end of request.
*   **Cache Locality**: objects are contiguous in memory.

```c
// 1. Create Arena at start of request
HwArena* arena = hw_arena_create(4096);

// 2. Allocate wildly without tracking individual pointers
char* url = hw_arena_alloc(arena, 128);
user_obj* u = hw_arena_alloc(arena, sizeof(user_obj));

// 3. Reset (keep memory) or Destroy (free memory) at end
