...
void http_server_destroy(http_server** srv) {
    ...
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (s->connections[i]) {
            ...
            if (s->connections[i]->arena)
                hw_arena_destroy(&s->connections[i]->arena);
...
static int http_server_init_net(http_server* srv) {
...
int http_server_is_running(const http_server* srv) {
    return srv ? atomic_load(&srv->running) : 0;
}
...
static int process_request(http_server* srv, http_conn* conn) {
    if (http_parse_request(conn->read_buf.data, conn->read_buf.len, &conn->req, conn->arena) != 0) {
        // HwBuffer full check could go here if implemented
    ...
    log_debug("[HTTP] %s %s (id=%.8s...)", conn->req.method, conn->req.path, conn->request_id);
    ...
    if (srv->async_handler) {
        ...
        (void) srv->handler(&conn->req, &conn->resp, srv->user_ctx);
    } else {
...
static int handle_read(http_server* srv, http_conn* conn) {
    ...
    if (!conn->req_parsed) {
        return process_request(srv, conn);
    }
...
static void generate_request_id(char* out) {
    ...
    snprintf(out, REQUEST_ID_LEN, "%08x-%04x-%04x-%04x-%08x%04x", (unsigned int) time(NULL),
             ...
             (unsigned int) (getpid() & 0xFFFF), ((c * 2654435761U) & 0xFFFFFFFF),
