...
json_value* json_parse(const char* str) {
    return (json_value*) cJSON_Parse(str);
}
...
const char* json_string_value(const json_value* v) {
    if (!cJSON_IsString((const cJSON*) v))
        return NULL;
    return ((const cJSON*) v)->valuestring;
}

double json_number_value(const json_value* v) {
    if (!cJSON_IsNumber((const cJSON*) v))
        return 0.0;
    return ((const cJSON*) v)->valuedouble;
}

bool json_bool_value(const json_value* v) {
    return cJSON_IsTrue((const cJSON*) v);
}
...
json_value* json_object_new(void) {
    return (json_value*) cJSON_CreateObject();
}

json_value* json_array_new(void) {
    return (json_value*) cJSON_CreateArray();
}

json_value* json_string_new(const char* s) {
    return (json_value*) cJSON_CreateString(s);
}

json_value* json_number_new(double n) {
    return (json_value*) cJSON_CreateNumber(n);
}

json_value* json_bool_new(bool b) {
    return (json_value*) cJSON_CreateBool(b);
}

json_value* json_null_new(void) {
    return (json_value*) cJSON_CreateNull();
}
...
char* json_stringify(const json_value* v) {
    return cJSON_PrintUnformatted((const cJSON*) v);
}

char* json_stringify_pretty(const json_value* v) {
    return cJSON_Print((const cJSON*) v);
}

bool json_is_array(const json_value* v) {
    return cJSON_IsArray((const cJSON*) v);
}
bool json_is_object(const json_value* v) {
    return cJSON_IsObject((const cJSON*) v);
}
bool json_is_string(const json_value* v) {
    return cJSON_IsString((const cJSON*) v);
}
bool json_is_number(const json_value* v) {
    return cJSON_IsNumber((const cJSON*) v);
}
