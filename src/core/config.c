...
int config_add_backend(config* cfg, const char* type, const char* path, bool primary) {
    if (!cfg || !type || !path)
        return -EINVAL;
    if (cfg->storage.backend_count >= CONFIG_MAX_BACKENDS)
        return -ENOSPC;
...
int config_load(config* cfg, const char* path) {
    ...
    if (storage) {
        ...
        if (backends && cJSON_IsArray(backends)) {
            ...
            if (count > CONFIG_MAX_BACKENDS)
                count = CONFIG_MAX_BACKENDS;
    ...
    if (loc) {
        ...
        if (lat)
            cfg->location.lat = lat->valuedouble;
        if (lon)
            cfg->location.lon = lon->valuedouble;
