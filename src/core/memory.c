...
void mem_set_scope_arena(HwArena *arena) { tls_arena = arena; }
...
void *mem_realloc(void *ptr, size_t size)
{
    ...
    if (tls_arena)
    {
        if (!ptr) return hw_arena_alloc(tls_arena, size);
...
{
    if (!ptr) return;
    if (tls_arena) return;  // No-op in arena mode
    free(ptr);
}
...
{
    if (item_size < sizeof(PoolNode)) item_size = sizeof(PoolNode);  // Enforce min size
...
{
    if (block_size < 1024) block_size = 1024;  // Min size
...
int hw_buffer_ensure_cap(HwBuffer *buf, size_t extra)
{
    ...
    {
        new_cap *= 2;
        if (new_cap < buf->cap) return -ENOMEM;  // Overflow
    }
