...
enum { MAX_PLUGINS = 32, MAX_PATH_LEN = 4096, MAX_ID_LEN = 256 };
...
static void stop_plugin(plugin_handle* h) {
    if (h->state != PLUGIN_STATE_RUNNING || h->pid <= 0)
        return;
...
plugin_type plugin_handle_type(const plugin_handle* h) {
    return h ? h->type : PLUGIN_TYPE_IN;
}
...
int plugin_mgr_check_health(plugin_mgr* mgr) {
    ...
    for (int i = 0; i < mgr->count; i++) {
        ...
        if (h->state == PLUGIN_STATE_STOPPED || h->state == PLUGIN_STATE_FAILED)
            continue;
...
int plugin_mgr_validate_dependencies(const plugin_mgr* mgr, const char* report_path,
    ...
    for (int i = 0; i < mgr->count; i++) {
        const plugin_handle* h = &mgr->plugins[i];
        if (!h->requires_json || !cJSON_IsArray(h->requires_json))
            continue;
