...
static int handle_api_request(heimwatt_ctx* ctx, const http_request* req, http_response* resp) {
    ...
    if (strcmp(req->path, "/api/plugins") == 0) {
        api_plugins_list(ctx, resp);
        return 0;
    }

    if (strncmp(req->path, "/api/plugins/", 13) == 0) {
...
void heimwatt_run(heimwatt_ctx* ctx) {
    heimwatt_run_with_shutdown_flag(ctx, NULL);
}
...
static void handle_signal_event(heimwatt_ctx* ctx, int sfd) {
...
static void handle_new_connection(heimwatt_ctx* ctx) {
...
static void process_incoming_json(heimwatt_ctx* ctx, ipc_conn* conn, char* msg, size_t len) {
    char* p = msg;
    const char* end = NULL;
...
static void disconnect_client(heimwatt_ctx* ctx, ipc_conn* conn) {
    int conn_idx = -1;
    pthread_mutex_lock(&ctx->conn_lock);
    for (int j = 0; j < ctx->conn_count; j++) {
        if (ctx->conns[j] == conn) {
...
static void handle_client_event(heimwatt_ctx* ctx, ipc_conn* conn, uint32_t ev) {
    int disconnect = 0;
    ...
    if (!disconnect && (ev & EPOLLIN)) {
        ...
        if (res < 0) {
            ...
            process_incoming_json(ctx, conn, msg, len);
            free(msg);
    ...
    if (disconnect) {
        disconnect_client(ctx, conn);
    }
}

void heimwatt_run_with_shutdown_flag(heimwatt_ctx* ctx,
    ...
    while (atomic_load(&ctx->running) && !g_shutdown && !(shutdown_flag && *shutdown_flag)) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (ptr == &SIGNAL_FD_TAG) {
                handle_signal_event(ctx, sfd);
            } else if (ipc_server_is_listen_event(ctx->ipc, ptr)) {
                handle_new_connection(ctx);
            } else {
                handle_client_event(ctx, (ipc_conn*) ptr, ev);
            }
        }
